...
function add(){
  ...
  if(['/auth.html','/admin.html','/trust.html','/architecture.html'].includes(path))return;
  ...
  const target=document.querySelector('.top-actions')||document.querySelector('.topbar')||document.body;if(target)target.prepend(a);
  refresh(a);
}
