...
async function refresh(a){const t=sessionStorage.getItem(K);if(!t)return;try{const r=await fetch('/v1/auth/me',{headers:{authorization:'Bearer '+t},cache:'no-store'});if(!r.ok){sessionStorage.removeItem(K);if(location.pathname!=='/auth.html'&&location.pathname!=='/admin.html')location.replace('/auth.html');return}const d=await r.json();a.href='/auth.html';a.innerHTML='<span class="nexa-account-dot"></span><span>'+esc(d.user.name)+'</span><button type="button" class="nexa-account-out">Sign out</button>';a.querySelector('button').onclick=async e=>{e.preventDefault();try{await fetch('/v1/auth/logout',{method:'POST',headers:{authorization:'Bearer '+t}})}finally{sessionStorage.removeItem(K);location.replace('/auth.html')}}}catch{}}
